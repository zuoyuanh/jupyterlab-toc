...
export function notebookGeneratorToolbar(
  ...
  options.notebookChanged = true;
  if (!options.preRenderedToolbar) {
    options.preRenderedToolbar = class extends React.Component<
    ...
    > {
      ...
      componentWillReceiveProps(nextProps: NotebookGeneratorToolbarProps) {
        if (nextProps.options.notebookChanged === true) {
          this.updateNotebook();
        }
      }

      updateNotebook() {
        this.props.options.notebookChanged = false;
  ...
  return options.preRenderedToolbar;
}
